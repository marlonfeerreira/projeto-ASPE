...
<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $nomeDiscente = $_POST["nome_discente"];
        $matricula = $_POST["matricula"];
        $curso = $_POST["curso"];
        $dataSolicitacao = $_POST["data_solicitacao"];
        $solicitante = $_POST["solicitante"];
        $setorSolicitante = $_POST["setor_solicitante"];
        $periodo = $_POST["periodo"];
        $modalidade = $_POST["modalidade"];
        $ano = $_POST["ano"];
        $relatoSolicitacao = $_POST["relato_solicitacao"];
  ...
  <div class="forms-registro">
    <form  method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
      ...
      <div class="p-12 flex flex-col gap-8">
        <p class="text-[28px] font-medium text-[rgba(63, 63, 63, 0.87)]">Registro de Atendimento ao Discente</p>
